...
class adpAEFusionLinear(nn.Module):
    ...
    def make_en(self, block, fusion_dim, layers_list, use_relu):
        ...
        for i in range(1, len(layers_list)):
            # nn.Sequential(
            #     nn.Linear(layers_list[i - 1], layers_list[i]),
            #     nn.BatchNorm1d(layers_list[i]),
            #     nn.ReLU(),
            # )
        ...
        layers.append(block(layers_list[-1], fusion_dim, use_relu=True))
            # nn.Sequential(
            #                 nn.Linear(layers_list[-1], fusion_dim),
    ...
    def make_de(self, block, fusion_dim, layers_list, use_relu):
        ...
        # nn.Sequential(
        #     nn.Linear(fusion_dim, layers_list[0]),
        #     nn.BatchNorm1d(layers_list[0]),
        #     nn.ReLU(),
        # )
...
def initAUXCFAndAE111(args):
    logging.info("T SELF method: " + str(args.self_method_t))
    logging.info("S SELF method: " + str(args.self_method_s))
    poolSize = [1, 1]
    args.auxCF_t = {}
    args.auxCF_s = {}
    block_method_t = args.aux_method_t
    block_method_s = args.aux_method_s
    logging.info("T block method: " + str(block_method_t))
    logging.info("S block method: " + str(block_method_s))
    ...
    logging.info("Fusion Feature Method: " + str(args.fusion_size) + "\tFusion Feature Size: " + str(fusion_dim))
    fusion_method_t = args.fusion_method_AE_t
    fusion_method_s = args.fusion_method_AE_s
    use_relu = args.ende_use_relu
    logging.info("T fusion AE method: " + str(fusion_method_t))
    logging.info("S fusion AE method: " + str(fusion_method_s))
    logging.info("USE RELU: " + str(use_relu))
    ...
    # elif args.fusion_method_AUXCF == 'ADPAELinear':
    #     # Adaptive AE Linear
    #     pass
...
def selectFusionMethod(in_channel, out_channel, fusion_dim, use_relu, fusion_method, args, is_T=True):
    if fusion_method == 'AEConv':
        # Conv2d 1x1 Single
        ae = aeFusionConv2Single(in_channel, out_channel, fusion_dim, use_relu=use_relu).cuda()
    elif fusion_method == 'AEConv3x3Single':
        # Conv2d 3x3 Single
        ae = aeFusionConv23x3Single(in_channel, out_channel, fusion_dim, use_relu=use_relu).cuda()
    elif fusion_method == 'AEConv3x3Linear':
        # Conv2d 3x3 + Linear
        ae = aeFusionConv2(in_channel, out_channel, fusion_dim, use_relu=use_relu).cuda()
    elif fusion_method == 'AELinear':
        # Linear
        ae = aeFusionLinear(in_channel, out_channel, fusion_dim, use_relu=use_relu).cuda()
        ...
        if is_T:
            logging.info("T Model: " + str(args.model_t) + ", AE_T Encoder Layers Number: " + str(len(input_list)))
        else:
            logging.info("S Model: " + str(args.model_s) + ", AE_S Encoder Layers Number: " + str(len(input_list)))

        if fusion_method == 'ADPAELinear':
            ae = adpAEFusionLinear(in_channel, fusion_dim, input_list, use_relu=use_relu).cuda()
        elif fusion_method == 'ADPAEConv3x3Linear':
            ae = adpAEFusionConv3x3Linear(in_channel, fusion_dim, input_list, use_relu=use_relu).cuda()
        elif fusion_method == 'ADPAEConv1x1Linear':
            ae = adpAEFusionConv1x1Linear(in_channel, fusion_dim, input_list, use_relu=use_relu).cuda()
    ...
    def forward(self, feat_t_fwd_list, feat_s_fwd_list, output_s_fwd, fwd_labels, criterion_cls, criterion_div, model_auxcfae_t, model_auxcfae_s, args):
        ...
        if args.NO_FUSION is True:
            ...
            if args.in_criterion == 'MSE':
                ...
                # t_rep = F.normalize(t_fusion.pow(2).mean(1).view(t_fusion.size(0), -1))
                # s_fusion = F.normalize(s_fusion.pow(2).mean(1).view(s_fusion.size(0), -1))
                # loss_fusion_fwd = (t_rep - s_fusion).pow(2).mean()
    ...
    def forward(self, feat_t_fb_list, feat_s_fb_list, output_t_fb, fb_labels, criterion_cls, criterion_div, loss_div_fb, model_t_weights, model_auxcfae_t, model_auxcfae_s, args):
        ...
        if args.NO_FUSION is True:
            ...
            if args.in_criterion == 'MSE':
                ...
                # t_rep = F.normalize(t_fusion.pow(2).mean(1).view(t_fusion.size(0), -1))
                # s_fusion = F.normalize(s_fusion.pow(2).mean(1).view(s_fusion.size(0), -1))
                # loss_fusion_fb = (t_rep - s_fusion).pow(2).mean()
                t_rep = F.normalize(t_fusion, dim=-1)
                s_fusion = F.normalize(s_fusion, dim=-1)
...
def cal_each_grad_sim3(cls_grad, other_grad, args):
    ...
    # cls_grad = torch.cat([each.reshape(-1) for each in cls_grad], -1).cuda()
    # other_grad = torch.cat([each.reshape(-1) for each in other_grad], -1).cuda()
...
def main():
    ...
    basic_net1 = inAuxCF(layers_list_t, [1, 1], 'Basic', 100)
    basic_net2 = inAuxCF(layers_list_t, [1, 1], 'Basic_133', 100)
    basic_net3 = inAuxCF(layers_list_t, [1, 1], 'Basic_313', 100)
    basic_net4 = inAuxCF(layers_list_t, [1, 1], 'Basic_313_another', 100)
    output = basic_net1(torch.randn(3, 64, 16, 16))
    print(123)
