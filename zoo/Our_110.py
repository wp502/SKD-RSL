...
__all__ = ["Our_FWD_Conv_110", "Our_FB_Conv_110", "initAUXCFAndAE110"]
...
class Our_FWD_Conv_110(nn.Module):
    """ 用于计算 forward 过程中的 中间特征层 的 loss """

    def __init__(self, T, args):
        super(Our_FWD_Conv_110, self).__init__()
        self.T = 1.0
    ...
    def forward(self, feat_t_fwd_list, feat_s_fwd_list, output_t_fwd, output_s_fwd, fwd_labels, criterion_cls, criterion_div,  args):
        ...
        loss_cls_ce = criterion_cls(output_s_fwd, fwd_labels)
        loss_.append(loss_cls_ce)
        args.loss_csv_fwd.append(loss_cls_ce.item())
        for each in s_reshape_list:
            temp_kl = criterion_div(each, output_s_fwd.detach())
            loss_.append(temp_kl)
            args.loss_csv_fwd.append(temp_kl.item())
...
class Our_FB_Conv_110(nn.Module):
    """ 用于计算 feedback 过程中的 中间特征层 的 loss """

    def __init__(self, T, args):
        super(Our_FB_Conv_110, self).__init__()
        self.T = 1.0
    ...
    def forward(self, feat_t_fb_list, feat_s_fb_list, output_t_fb, output_s_fb, fb_labels, criterion_cls, criterion_div, loss_div_fb, model_t_weights, args):
        ...
        for t_1 in t_reshape_list:
            ...
            #         args.loss_csv_fb.append(temp_kl.item())
                    ...
                    # else:
                    #     loss_kl_.append(temp_kl)
        loss_cls_ce = criterion_cls(output_t_fb, fb_labels)
        loss_ce_.append(loss_cls_ce)
        args.loss_csv_fb.append(loss_cls_ce.item())
        for each in t_reshape_list:
            temp_kl = criterion_div(each, output_t_fb.detach())
            loss_kl_.append(temp_kl)
            args.loss_csv_fb.append(temp_kl.item())
        ...
        loss_recon_fb_t_list = []
        loss_recon_fb_s_list = []
        # 重构损失
        for each_t, each_recon in zip(t_ae_list, t_recon_split):
            loss_recon_fb_t_list.append(F.mse_loss(each_t, each_recon))
        for each_s, each_recon in zip(s_ae_list, s_recon_split):
            loss_temp = F.mse_loss(each_s, each_recon)
            loss_recon_fb_s_list.append(loss_temp)
            # if loss_temp < 10000.0:
            #     loss_recon_fb_s_list.append(loss_temp)

        loss_recon_fb_t = sum(loss_recon_fb_t_list)
        loss_recon_fb_s = sum(loss_recon_fb_s_list)

        args.loss_csv_fb.append(loss_recon_fb_t.item())
        if loss_recon_fb_s is not 0:
            args.loss_csv_fb.append(loss_recon_fb_s.item())
        else:
            args.loss_csv_fb.append(str(0))
