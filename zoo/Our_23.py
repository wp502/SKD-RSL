...
__all__ = ["Our_FWD_Conv_23", "Our_FB_Conv_23"]
...
class AEFusion(nn.Module):
    """ forward过程中，经过卷积自编码器融合知识。接收者为 student， feedback过程中，接收者为 teacher """

    def __init__(self, input_channel, receiver_channel, use_relu=True):
        super(AEFusion, self).__init__()
        self.use_relu = use_relu
        self.en = nn.Conv1d(in_channels=input_channel, out_channels=receiver_channel, kernel_size=1, stride=1, padding=0,
                            bias=True)
        self.bn1 = nn.BatchNorm1d(receiver_channel)
        self.relu = nn.ReLU(inplace=True)
        self.de = nn.Conv1d(in_channels=receiver_channel, out_channels=input_channel, kernel_size=1, stride=1, padding=0,
                            bias=True)
...
class Our_FWD_Conv_23(nn.Module):
    """ 用于计算 forward 过程中的 中间特征层 的 loss """

    def __init__(self, T, args):
        super(Our_FWD_Conv_23, self).__init__()
        self.T = 1.0
        self.fwd_ae = AEFusion(sum(args.t_shape, args.s_shape), args.s_shape, args.ende_use_relu)
    ...
    def forward(self, feat_t_fwd_list, feat_s_fwd_list, args):
        ...
        ts_cat = torch.cat([t_cat, s_cat], 1)

        ts_fusion, ts_recon = self.fwd_ae(ts_cat)
        ...
        loss_chfwd_dict = dict(recon_T_fwd=loss_recon_fwd_t, recon_S_fwd=loss_recon_fwd_s, fusion_fwd=loss_fusion_fwd)
        return loss_chfwd_dict


class Our_FB_Conv_23(nn.Module):
    """ 用于计算 feedback 过程中的 中间特征层 的 loss """

    def __init__(self, T, args):
        super(Our_FB_Conv_23, self).__init__()
        self.T = 1.0
    ...
    def forward(self, feat_t_fb_list, feat_s_fb_list, args):
        ...
        loss_chfb_dict = dict(recon_T_fb=loss_recon_fb_t, recon_S_fb=loss_recon_fb_s, fusion_fb=loss_fusion_fb)
