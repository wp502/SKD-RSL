...
__all__ = ["Our_FWD_Conv_16_2", "Our_FB_Conv_16_2"]
...
class Our_FWD_Conv_16_2(nn.Module):
    """ 用于计算 forward 过程中的 中间特征层 的 loss """

    def __init__(self, T, args):
        super(Our_FWD_Conv_16_2, self).__init__()
        self.T = 1.0
        ...
        self.in_dim = args.num_class * args.blocks_amount
    ...
    def forward(self, feat_t_fwd_list, feat_s_fwd_list, output_t_fwd, output_s_fwd, fwd_labels, criterion_cls, criterion_div,  args):
        ...
        for i in range(args.blocks_amount):
            feat_t_temp, out_t_temp = self.fwd_auxCF_t[str(i+1)](feat_t_fwd_list[i])
            feat_s_temp, out_s_temp = self.fwd_auxCF_s[str(i+1)](feat_s_fwd_list[i])
        ...
        t_ae_list = t_out_list[:-1]
        s_ae_list = s_out_list[:-1]
...
class Our_FB_Conv_16_2(nn.Module):
    """ 用于计算 feedback 过程中的 中间特征层 的 loss """

    def __init__(self, T, args):
        super(Our_FB_Conv_16_2, self).__init__()
        self.T = 1.0
    ...
    def forward(self, feat_t_fb_list, feat_s_fb_list, output_t_fb, output_s_fb, fb_labels, criterion_cls, criterion_div, loss_div_fb, model_t_weights, args):
        ...
        for i in range(args.blocks_amount):
            feat_t_temp, out_t_temp = self.fb_auxCF_t[str(i + 1)](feat_t_fb_list[i])
            feat_s_temp, out_s_temp = self.fb_auxCF_s[str(i + 1)](feat_s_fb_list[i])
            t_feat_list.append(feat_t_temp)
            t_out_list.append(out_t_temp)
            s_feat_list.append(feat_s_temp)
            s_out_list.append(out_s_temp)
