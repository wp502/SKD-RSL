...
__all__ = ["Our_FWD_Conv_12", "Our_FB_Conv_12"]
...
class Our_FWD_Conv_12(nn.Module):
    """ 用于计算 forward 过程中的 中间特征层 的 loss """

    def __init__(self, T, args):
        super(Our_FWD_Conv_12, self).__init__()
        self.T = 1.0
    ...
    def forward(self, feat_t_fwd_list, feat_s_fwd_list, args):
        ...
        if args.in_criterion == 'MSE':
            ...
            t_rep = F.normalize(t_rep, dim=-1)
            s_fusion = F.normalize(s_fusion, dim=-1)
            loss_fusion_fwd = F.mse_loss(t_rep, s_fusion)
        elif args.in_criterion == 'MSE_softmax':
            loss_fusion_fwd = F.mse_loss(F.softmax(t_rep, dim=-1), F.softmax(s_fusion, dim=-1))
            ...
            loss_fusion_fwd = criterion_int_kl(F.log_softmax(s_fusion / self.T, dim=-1),
                                           F.softmax(t_rep / self.T, dim=-1)) * (self.T * self.T) / (N * C)
...
class Our_FB_Conv_12(nn.Module):
    """ 用于计算 feedback 过程中的 中间特征层 的 loss """

    def __init__(self, T, args):
        super(Our_FB_Conv_12, self).__init__()
        self.T = 1.0
    ...
    def forward(self, feat_t_fb_list, feat_s_fb_list, args):
        ...
        if args.in_criterion == 'MSE':
            ...
            s_rep = F.normalize(s_rep, dim=-1)
            t_fusion = F.normalize(t_fusion, dim=-1)
            loss_fusion_fb = F.mse_loss(s_rep, t_fusion)
        elif args.in_criterion == 'MSE_softmax':
            loss_fusion_fb = F.mse_loss(F.softmax(s_rep, dim=-1), F.softmax(t_fusion, dim=-1))
        elif args.in_criterion == 'KL_softmax':
            # 因为展开后算上 batchsize 是三维，需要用 sum 后再平均
            criterion_int_kl = nn.KLDivLoss(reduction='sum')
            loss_fusion_fb = criterion_int_kl(F.log_softmax(t_fusion / self.T, dim=-1),
                                               F.softmax(s_rep / self.T, dim=-1)) * (self.T * self.T) / (N * C)
