        ...
        for t in self.target_type:
            if t == "category":
                ...
                data = scipy.io.loadmat(os.path.join(self.root,
                                                     "Annotations",
                                                     self.annotation_categories[self.targets[index]],
...
class Caltech256(VisionDataset):
    ...
    def __init__(self, root, transform=None, target_transform=None, train=True, download=False, test_percent=0.2):
        ...
        self.train_percent = 1.0-test_percent
        if download:
            self.download()
        ...
        for (i, c) in enumerate(self.categories):
            if i == 197 or i == 55:
                # no_use_file_count = files_list.count('RENAME2') # 198.spider
                # files_list.count('greg') # 056.dog
                no_use_file_count = 1
            else:
                no_use_file_count = 0

            n = len(os.listdir(os.path.join(self.root, "256_ObjectCategories", c))) - no_use_file_count
            # sum_all += n
            if 'clutter' in c:
                continue
            # print(i, c, n)
            train_range_end = round(n * self.train_percent)
            if self.train:
                self.index.extend(range(1, train_range_end+1))
                self.targets.extend(train_range_end * [i])
            else:
                self.index.extend(range(train_range_end+1, n+1))
                self.targets.extend((n - train_range_end) * [i])
    ...
    def __getitem__(self, index):
        ...
        img = Image.open(os.path.join(self.root,
                                      ...
                                      self.categories[self.targets[index]],
                                      "{:03d}_{:04d}.jpg".format(self.targets[index] + 1, self.index[index])))
        if img.mode=='L':
    ...
    def _check_integrity(self):
        # can be more robust and check hash of files
        print(os.path.join(self.root, "256_ObjectCategories"))
