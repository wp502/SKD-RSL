...
class CUB200(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        return image, target # image:数据 target:标签
    ...
    def _extract_folder(self):
        ...
        id2classid = np.genfromtxt(os.path.join(
            self._root, 'raw/CUB_200_2011/image_class_labels.txt'), dtype=int)
        # Format of classes.txt: <class_id> <class_name>
        classes_name = np.genfromtxt(os.path.join(
            self._root, 'raw/CUB_200_2011/classes.txt'), dtype=str)
        ...
        for idx, each_pth in enumerate(id2pth):
            ...
            img_is_train = id2train[1]
            source = os.path.join(image_path, each_pth)
